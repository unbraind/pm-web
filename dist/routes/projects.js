...
export async function verifyProjectAccess(userId, projectId) {
    ...
    const ownResult = await pool.query(`SELECT id, name, slug, description, prefix, user_id AS owner_user_id
     FROM pm_projects WHERE id = $1 AND user_id = $2`, [projectId, userId]);
    ...
    const shareResult = await pool.query(`SELECT p.id, p.name, p.slug, p.description, p.prefix, p.user_id AS owner_user_id,
     ...
     GROUP BY p.id, p.name, p.slug, p.description, p.prefix, p.user_id`, [projectId, userId]);
...
router.post("/", async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO pm_projects (user_id, name, slug, description, prefix)
       VALUES ($1, $2, $3, $4, $5) RETURNING id, name, slug, description, prefix, created_at`, [req.user.userId, name.trim(), slug, description?.trim() || "", prefix.trim()]);
