...
export async function findUserByEmail(email) {
    const result = await pool.query(`SELECT id, email, display_name FROM pm_users WHERE email = $1`, [email.trim().toLowerCase()]);
 ...
 */
export function notFoundWhenEmpty(res, rows, message) {
    if (rows.length === 0) {
 ...
 */
export function parseGroupInput(res, body) {
