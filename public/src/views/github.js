...
export async function renderGitHubView() {
...
function renderGitHubContent(data) {
    ...
    const contentEl = document.getElementById('github-content');
    if (contentEl)
        contentEl.innerHTML = linkPanel + syncPanels;
}
export async function linkGitHubRepo() {
    const owner = document.getElementById('gh-owner')?.value?.trim() || '';
    const repo = document.getElementById('gh-repo')?.value?.trim() || '';
    const errEl = document.getElementById('gh-link-error');
    const btn = document.getElementById('gh-link-btn');
    if (errEl)
        errEl.style.display = 'none';
    if (!owner || !repo) {
        if (errEl) {
            errEl.textContent = 'Owner and repository name are required';
    ...
    try {
        ...
        if (btn) {
            ...
            const sp = btn.querySelector('span');
            if (sp)
                sp.textContent = 'Link Repository';
        }
    }
}
export function unlinkGitHubRepo() {
        ...
        ${issues.map((i) => `
          <label style="display:flex;align-items:flex-start;gap:10px;padding:8px 10px;background:var(--bg-card2);border:1px solid var(--border);border-radius:var(--radius);cursor:pointer;transition:var(--transition)" onmouseover="this.style.borderColor='var(--border-light)'" onmouseout="this.style.borderColor='var(--border)'">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:11px;color:var(--text-muted);margin-top:2px">#${escHtml(String(i.number))}${i.labels?.length ? ` · ${i.labels.map((l) => escHtml(l.name || l)).join(', ')}` : ''}</div>
            </div>
          </label>`).join('')}
      </div>
...
export function selectAllIssues(checked) {
    document.querySelectorAll('.gh-issue-cb').forEach(cb => { cb.checked = checked; });
}
export async function loadItemsForPush() {
    ...
    try {
        const [itemsData, linksData] = await Promise.all([
            api('GET', `/projects/${state.currentProject.id}/pm/list-all?limit=200`),
            api('GET', `/projects/${state.currentProject.id}/github/links`),
        ]);
...
export function selectAllPushItems(checked) {
    document.querySelectorAll('.gh-push-cb').forEach(cb => { cb.checked = checked; });
}
export async function pushItemsToGitHub() {
    const checked = Array.from(document.querySelectorAll('.gh-push-cb:checked'));
    if (checked.length === 0) {
        toast('Select at least one item to push', 'info');
        return;
    }
    ...
    if (btn) {
        ...
        const sp = btn.querySelector('span');
        if (sp)
            sp.textContent = `Pushing ${itemIds.length}…`;
    }
    const resultEl = document.getElementById('github-push-result');
    if (resultEl)
        resultEl.style.display = 'none';
    try {
        const data = await api('POST', `/projects/${state.currentProject.id}/github/push`, { itemIds });
        ...
        if (btn) {
            ...
            const sp = btn.querySelector('span');
            if (sp)
                sp.textContent = 'Push to GitHub';
        }
    }
}
export async function updateGitHubIssue(itemId) {
    try {
        const data = await api('PATCH', `/projects/${state.currentProject.id}/github/push/${encodeURIComponent(itemId)}`);
...
export async function importGitHubIssues() {
    ...
    if (checked.length === 0) {
        toast('Select at least one issue to import', 'info');
        return;
    }
    const issueNumbers = checked.map(cb => parseInt(cb.dataset.number || '0', 10));
    const btn = document.getElementById('gh-import-btn');
    if (btn) {
        btn.disabled = true;
        const sp = btn.querySelector('span');
        if (sp)
            sp.textContent = `Importing ${issueNumbers.length}…`;
    }
    ...
    try {
        ...
        <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          ...
          ${errors.length > 0 ? `<div style="color:var(--status-blocked);font-size:13px">✗ ${errors.length} error${errors.length !== 1 ? 's' : ''}: ${errors.map((e) => escHtml(String(e.message || e))).join('; ')}</div>` : ''}
        </div>`;
        }
        ...
        if (btn) {
            ...
            const sp = btn.querySelector('span');
            if (sp)
                sp.textContent = 'Import Selected';
        }
    }
}
