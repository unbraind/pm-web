 ...
 *   IP/subnet string.
 */
export function resolveTrustProxy(env: NodeJS.ProcessEnv = process.env): number | boolean | string {
 ...
 */
function resolveLimit(env: NodeJS.ProcessEnv, key: string, fallback: number): number {
  const raw = env[key];
 ...
 */
export function createTierLimiters(env: NodeJS.ProcessEnv = process.env): TierLimiters {
