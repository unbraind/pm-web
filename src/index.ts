...
export function nodeVersionMeetsRequirement(version: string = process.versions.node): boolean {
  ...
  for (let i = 0; i < REQUIRED_NODE_VERSION.length; i += 1) {
    if (parsed[i] > REQUIRED_NODE_VERSION[i]) return true;
    if (parsed[i] < REQUIRED_NODE_VERSION[i]) return false;
  }
  return true;
}
...
async function probeHealthz(
  port: string,
): Promise<{ reachable: boolean; healthy: boolean; body?: unknown; error?: string }> {
